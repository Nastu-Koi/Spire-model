...
def resumable_jobs(previous):
    ...
    for job in manifest['jobs']:
        ...
        finished = summary.is_file() and json.loads(summary.read_text()).get('status') == 'verified_victory'
        if frontier.is_file() and not finished:
            jobs.append({'character': character, 'seed': seed, 'resume': str(frontier.resolve())})
    if not jobs:
...
def main():
    parser = argparse.ArgumentParser(description='Resume unfinished Bootstrap trajectory jobs')
    ...
    parser.add_argument('--reuse-turn-plan', action=argparse.BooleanOptionalAction, default=None)
    args = parser.parse_args()
    jobs, manifest = resumable_jobs(args.previous)
    prior = manifest['options']
    options = dict(
        config=args.config or Path(manifest.get('config', DEFAULT_CONFIG)), jobs=jobs, output=args.output, workers=args.workers,
        ascension=prior.get('ascension', 10),
        search_lanes=(args.search_lanes if args.search_lanes is not None
                      else prior.get('search_lanes', 2)),
        budget_ms=args.budget_ms if args.budget_ms is not None else prior.get('budget_ms', 2500),
        weight=args.weight if args.weight is not None else prior.get('weight', 12),
        max_expansions=args.max_expansions, max_seconds=args.max_seconds, max_steps=args.max_steps,
        rollout_decisions=(args.rollout_decisions if args.rollout_decisions is not None
                           else prior.get('rollout_decisions', 256)),
        reuse_turn_plan=(args.reuse_turn_plan if args.reuse_turn_plan is not None
