...
def generate(config, jobs, output, *, workers=1, **options):
    ...
    if not identities or len(set(identities)) != len(identities): raise ValueError('Jobs must be nonempty and unique')
    if any(c not in CHARACTERS for c, s in identities): raise ValueError('Unsupported character')
    ...
    write_json(output/'manifest.json', dict(config=str(config), jobs=jobs, workers=workers, options=options))
    results, accepted = [], []
    for job, (character, seed) in zip(jobs, identities):
        ...
        for key in ('resume', 'prefix_path'):
            if job.get(key): job_options[key] = Path(job[key])
        try:
            ...
            if result['status'] == 'verified_victory':
                ...
                if len(runs) != 1 or runs[0]['provenance'].get('verified_outcome') != f'A{ascension}_final_boss_victory':
                    raise ValueError('Missing verified outcome')
                accepted.extend(runs)
        except Exception as exc:
            result = dict(character=character, seed=seed, status='job_error', error=str(exc))
    ...
    with temporary.open('x') as stream:
        for run in accepted: stream.write(json.dumps(run, allow_nan=False)+'\n')
    temporary.replace(output/'accepted.jsonl')
    summary = dict(ascension=ascension, verified_victories=len(accepted), unresolved_or_defeated=len(jobs)-len(accepted), jobs=results)
