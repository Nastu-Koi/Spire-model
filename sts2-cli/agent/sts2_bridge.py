    ...
    if isinstance(obj, dict):
        ...
        for k, v in obj.items():
            ...
            if k == "relics" and isinstance(v, list) and depth > 0:
                result[k] = [
                    compact_json(r, depth + 1) if isinstance(r, dict) else r for r in v
                ]
                continue
...
def do_replay():
    if not REPLAY_FILE:
        print(
            "Usage: sts2_bridge.py replay <logfile> [--until STEP] [--port PORT]",
            file=sys.stderr,
        )
        sys.exit(1)

    print(
        f"Replaying {REPLAY_FILE} until step {REPLAY_UNTIL or 'end'}...",
        file=sys.stderr,
    )
    game = Game()
    ...
    for entry in entries:
        ...
        hp = (
            resp.get("hp", resp.get("player", {}).get("hp", ""))
    ...
    if last_resp:
        ...
        print(
            json.dumps(last_resp, ensure_ascii=False, indent=2)[:2000], file=sys.stderr
        )
    ...
    class ReplayHandler(BaseHTTPRequestHandler):
        def do_POST(self):
            ...
            _step_num, result = game.send(data)
