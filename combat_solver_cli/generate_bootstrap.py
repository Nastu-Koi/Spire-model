...
def make_jobs(seeds_per_character, characters=CHARACTERS, seed_prefix=None):
    ...
    return [{'character': character, 'seed': seed}
            for seed in seeds for character in characters]
...
def main():
    ...
    parser.add_argument('--seed-prefix', help='Use deterministic seeds for a reproducible experiment')
    ...
    parser.add_argument('--reuse-turn-plan', action=argparse.BooleanOptionalAction, default=True)
    args = parser.parse_args()
    if args.seeds_per_character is not None:
        jobs = make_jobs(args.seeds_per_character, args.characters, args.seed_prefix)
        options = vars(args).copy()
        options.pop('seed_prefix'); options.pop('seeds_per_character')
        options.pop('target_trajectories'); options.pop('characters')
        result = generate(jobs=jobs, **options)
        success = bool(result['verified_victories'])
    else:
        target = args.target_trajectories if args.target_trajectories is not None else 100
        options = vars(args).copy()
        for key in ('seed_prefix', 'seeds_per_character', 'target_trajectories', 'characters'):
            options.pop(key)
        if args.seed_prefix:
            raise ValueError('--seed-prefix is only available with --seeds-per-character; target mode uses one random seed per trajectory')
        result = generate_target(target_trajectories=target, characters=args.characters,
                                 **options)
