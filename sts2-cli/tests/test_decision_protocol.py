...
def execute(game, frame, candidate=None, **overrides):
    ...
    command = {
        ...
        "candidate_ref": (candidate or frame["legal"]["candidates"][0])[
            "candidate_ref"
        ],
    }
...
def test_query_stable_and_rejected_commands_do_not_edit_prefix(game):
    ...
    assert (
        result["routing"]["base_public_version"]
        == frame["routing"]["base_public_version"]
    )
    assert (
        result["routing"]["action_bank_version"]
        == frame["routing"]["action_bank_version"]
    )
...
def test_purity_stop_exhausts_only_selected_cards(game, count):
    ...
    for _ in range(count):
        candidate = next(
            c for c in frame["legal"]["candidates"] if c["verb"] == "SELECT_ONE"
        )
        ref = candidate["source_refs"][0]
        selected.append(
            next(
                c["content_id"]
                for c in frame["public"]["entities"]
                if c.get("ref") == ref
            )
        )
        old = frame
        ...
        assert ref not in [
            ...
            if c["verb"] == "SELECT_ONE"
        ]
    stop = next(
        c for c in frame["legal"]["candidates"] if c["verb"] == "FINISH_SELECTION"
    )
...
def test_optional_single_card_keeps_stop_candidate(game):
    ...
    frame = advance(game)
    assert [c["verb"] for c in frame["legal"]["candidates"]] == [
        "SELECT_ONE",
        "FINISH_SELECTION",
    ]
