...
def configuration(path=DEFAULT_CONFIG):
    try:
        ...
        for key in ("solver_dll", "game_dll"):
            if (
                hashlib.sha256(Path(config[key]).read_bytes()).hexdigest()
                != config[key + "_sha256"]
            ):
                raise InfrastructureError(
                    f"{key} changed; configure and validate again"
                )
        if not Path(config["worker_dll"]).is_file():
            raise InfrastructureError("Build the worker first")
        return config
...
class SolverEngine(CliEngine):
    def __init__(self, config=DEFAULT_CONFIG, timeout=150):
        ...
        if not launcher.is_file():
            raise InfrastructureError(f"Worker launcher disappeared: {launcher}")
        try:
            super().__init__(
                [sys.executable, str(launcher), str(config)],
                root=ROOT / "sts2-cli",
                timeout=timeout,
            )
        ...
        *,
        ...
        return self.send(
            dict(
                ...
                beam_portfolio=beam_portfolio,
                **(
                    {"boss_budget_ms": boss_budget_ms}
                    if boss_budget_ms is not None
                    else {}
                ),
    ...
    def solve(self, frame, **options):
        return self.solver("solver_solve", frame, **options)

    def step(self, frame, **options):
        return self.solver("solver_step", frame, **options)
