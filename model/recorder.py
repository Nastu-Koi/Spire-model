...
def card_fields(raw):
    ...
    if result["content_id"] == "CARD.MAD_SCIENCE":
        ...
        require(
            isinstance(variant.get("rider_effect"), str)
            and bool(variant["rider_effect"])
...
class PublicSnapshot:
    def __init__(self, state):
        ...
        if combat.get("in_progress"):
            ...
            self.entities.append(
                {"entity_type": "orb_slots", "capacity": pcs.get("orb_slots")}
            )
                ...
                self.entities.append(
                    {
                        ...
                        "stats": {
                            "passive": orb.get("passive_value"),
                            "evoke": orb.get("evoke_value"),
                        },
                    }
                )
    ...
    def creature(self, raw, kind, ref):
        self.add(
            ...
            ref,
            {
                "content_id": content_id(raw, "monster"),
                "known": True,
    ...
    def context(self, legal):
        ...
        if scope == "event_choice":
            self.entities.append(
                {"entity_type": "event", "content_id": "EVENT." + context["event_id"]}
            )
            for i, option in enumerate(context["options"]):
                self.add(
                    ...
                    f"option:{i}",
                    {
                        "content_id": option["text_key"],
                        "enabled": not option.get("locked", False),
                        "effect_coverage": "opaque",
            ...
            self.entities.append(
                {
                    "entity_type": "event_state",
                    "content_id": "CRYSTAL_SPHERE",
                ...
                self.entities.append(
                    {
                        ...
                        "semantic_program": {
                            "kind": "effect",
                            "op": "REVEAL",
                            "shape": "cell" if tool == "Small" else "square",
            ...
            for cell in context["cells"]:
                ...
                fields = {
                    ...
                    "x": cell["x"],
                    "y": cell["y"],
                    "revealed": not cell["hidden"],
                    "known": not cell["hidden"],
                }
                if not cell["hidden"]:
                    item = cell.get("item")
                    fields["content_id"] = (
                        item.get("type", "unknown").rsplit(".", 1)[-1]
                        if item
                        else "empty"
                    )
    ...
    def action(self, raw, index, scope):
        ...
        result = {
            ...
            "decoder_slot_ref": f"a{index}",
            "verb": VERBS[command],
        }
        if (
            command in {"pick_relic", "select_reward_option", "select_bundle"}
            ...
            if source is not None:
                ...
                require(
                    any(e.get("ref") == result["source_ref"] for e in self.entities),
                    "Missing visible map node",
                )
            elif command in {
                "choose_rest_option",
                "pick_relic",
                "select_reward_option",
                "select_bundle",
            }:
                result["source_ref"] = self.keys[(scope, args["index"])]
                if command == "select_reward_option" and result[
                    "source_ref"
...
def decision_macro(decision, run_id, contract):
    ...
    require(
        decision.get("state_boundary")
        in {"execution_or_semantic_entry", "input_submission", "selection_offer"},
    ...
    require(
        ...
        "Missing complete recorded legal set",
    )
    actions = legal.get("actions", [])
    require(
        actions and len({fingerprint(x) for x in actions}) == len(actions),
        "Empty or duplicate recorded legal actions",
    )
    scope = legal["scope"]
    ...
    if choice and choice["command"] == "select_cards":
        ...
        require(
            selection
            and selection.get("min") == selection.get("max") == 1
            and len(ids) == 1,
    ...
    require(
        len(matches) == 1, "Recorded choice is not uniquely in the complete legal set"
    )
    require(
        decision.get("legal_match", {}).get("status") == "matched",
        "Recorder did not verify choice legality",
    )
    public = {
        "phase": PHASES[scope],
    ...
    frame = {
        ...
        "routing": {
            ...
            "state_version": decision["order"],
        },
        "public": public,
        "legal": {"candidates": bank},
        "events": [],
    }
    return {
        "phase": PHASES[scope],
        "steps": [
            {
                ...
                "candidate_ref": bank[matches[0]]["candidate_ref"],
                "forced": len(bank) == 1,
            }
        ],
    }
...
def selection_macro(decision, run_id, contract, snapshot):
    ...
    require(
        path.get("mode") == "buffered"
        and path.get("reconstructed") is True
        and path.get("order_source") == "submitted_result",
        "Unknown selection path semantics",
    )
    require(
        decision["choice_key"]["command"] == "select_cards",
        "Selection path label is not a card set",
    )
    require(
        decision.get("legal_match", {}).get("status") == "matched",
        "Invalid recorded selection",
    )
    offered = legal["actions"]
    require(
        all(a["command"] == "select_card" for a in offered),
        "Unexpected action in selection offer",
    )
    ...
    require(
        ...
        and type(cancelable) is bool,
        "Invalid selection bounds",
    )
    require(
        len(result) == len(set(result))
        and set(result) <= set(ids)
        and len(result) <= maximum
        and (len(result) >= minimum or (not result and cancelable)),
        "Invalid submitted selection",
    )
    finish, cancel = (
    ...
    for revision, part in enumerate(path["steps"]):
        ...
        require(
            part["selected_ids"] == prefix,
            "Selection prefix disagrees with submitted order",
        )
        expected = (
            [a for a in offered if a["args"]["card_instance_id"] not in prefix]
        ...
        require(
            part["choice_key"] == label and label in expected,
            "Selection prefix label is illegal",
        )
        ...
        context = {
            ...
            "min_total": minimum,
            "max_total": maximum,
            "selected_refs": [snapshot.refs[x] for x in prefix],
            "selected_count": len(prefix),
            "remaining_required": max(0, minimum - len(prefix)),
            "can_finish": finish in expected,
            "can_cancel": cancel in expected,
            "can_skip": False,
        ...
        frame = {
            ...
            "routing": {
                "episode_id": run_id,
                "decision_id": f"{decision['action_id']}:{revision}",
                "state_version": revision,
                "selection_id": decision["action_id"],
                "selection_revision": revision,
                "base_public_version": decision["order"],
        ...
        steps.append(
            {
                "frame": clean_frame(frame),
                "candidate_ref": bank[raw_bank.index(label)]["candidate_ref"],
    ...
    return {
        "phase": "card_select",
        "steps": steps,
        "label_path": "reconstructed_from_submitted_result",
    }
...
def verify_teachers(decisions):
    ...
    def public_teacher(decision, seen):
        ...
        if evidence.startswith("parent_action:"):
            ...
            return parent is not None and public_teacher(
                parent, seen | {decision["action_id"]}
            )
        return False

    require(
        all(public_teacher(d, set()) for d in decisions),
...
def read_journal(path):
    ...
    with Path(path).open("rb") as stream:
        for number, raw in enumerate(stream, 1):
            ...
            row = json.loads(raw)
            require(
                isinstance(row, dict) and row.get("schema") == RECORDER_SCHEMA,
                f"Unsupported recorder schema on line {number}",
            )
            require(row.get("seq") == number, f"Journal sequence gap on line {number}")
            require(
                row.get("run_id") and row.get("segment_id"), "Missing journal identity"
            )
            if rows:
                require(
                    (row["run_id"], row["segment_id"])
                    == (rows[0]["run_id"], rows[0]["segment_id"]),
...
def convert_journal(path, *, bc_only=False, recorder_version=None):
    ...
    if recorder_version is not None:
        require(
            ...
            "Recorder version does not match filter",
        )
    endings = [row["data"] for row in rows if row["kind"] == "run_ended"]
    if not bc_only:
        require(
            start.get("begins_at_run_start") is True,
            "Resumed fragment; complete-run continuity is unverified",
        )
        require(
            rows[-1]["kind"] == "segment_end",
            "Recording is active or truncated; no segment end",
        )
        require(
            rows[-1]["data"].get("uncommitted_operations") == 0,
            "Recording has uncommitted decisions",
        )
        require(
            rows[-1]["data"].get("state_capture_errors", 0) == 0,
            "Recording contains failed state captures",
        )
        require(
            len(endings) == 1 and type(endings[0].get("victory")) is bool,
            "Run has no unique terminal outcome",
        )
        ...
        require(
            rows[-1]["data"].get("run_ended") is True,
            "Segment end does not confirm run completion",
        )
        allowed = {"segment_start", "segment_end", "run_ended", "decision_committed"}
        require(
            all(row["kind"] in allowed for row in rows),
            "Recording contains errors or an unsupported event format",
        )
        require(
            sum(row["kind"] == "segment_start" for row in rows) == 1,
    ...
    decisions = [
        item
        for row in rows
        if row["kind"] == "decision_committed"
        for item in flatten(row["data"])
    ]
    require(decisions, "Recording has no decisions")
    decisions.sort(key=lambda d: d["order"])
    require(
        len({d["action_id"] for d in decisions}) == len(decisions),
        "Repeated decision identity",
    )
    require(
        len({d["order"] for d in decisions}) == len(decisions),
        "Ambiguous decision order",
    )
    ...
    contract = {
        ...
        "game_assembly_mvid": next(
            (
                a["mvid"]
                for a in start.get("loaded_assemblies", [])
                if a["name"] == "sts2"
            ),
            None,
        ),
    ...
    for decision in decisions:
        try:
            require(
                decision["state"]["players"][0]["character"] == character.upper(),
                "Character changed within recording",
            )
            require(
                str(decision["state"]["run"].get("seed")) == str(start["seed"]),
...
def import_recordings(paths, output, *, bc_only=False, recorder_version=None):
    ...
    with (
        (directory / "accepted.jsonl").open("x", encoding="utf-8") as good,
        ...
        for path in sorted(files):
            try:
                if recorder_version is not None:
                    ...
                    if (
                        header.get("data", {}).get("recorder_version")
                        != recorder_version
                    ):
                        counts["version_filtered_files"] += 1
                        continue
                run = convert_journal(
                    path, bc_only=bc_only, recorder_version=recorder_version
                )
                require(
                    run["run_id"] not in identities, "Duplicate complete-run identity"
                )
                ...
                counts["rejected_decisions"] += len(
                    run["provenance"].get("rejected_decisions", [])
                )
                ...
                bad.write(
                    json.dumps(
                        {"path": str(path.resolve()), "reason": reason},
                        ensure_ascii=False,
                    )
                    + "\n"
                )
    summary = {
        "importer": IMPORT_VERSION,
        "bc_only": bc_only,
        "recorder_version_filter": recorder_version,
        "files": len(files),
        "accepted_runs": 0,
        **dict(counts),
        "quarantine_reasons": dict(reasons),
        "accepted": str(directory / "accepted.jsonl"),
