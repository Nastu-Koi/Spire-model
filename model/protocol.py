...
def validate_frame(frame: dict, *, allow_prototype: bool = False) -> dict:
    ...
    if boundary == "terminal" and type((frame.get("public", {}).get("outcome") or {}).get("victory")) is not bool:
    ...
    if routing.get("selection_id") and type(routing.get("selection_revision")) is not int:
    ...
    if context.get("mode") == "buffered":
        ...
        if context.get("repetition_allowed") is False and len(set(selected)) != len(selected):
            raise ProtocolError("Repeated reference in non-repeating selection")
        controls = {"can_finish": "FINISH_SELECTION", "can_skip": "SKIP", "can_cancel": "CANCEL"}
        ...
        for field, verb in controls.items():
            if type(context.get(field)) is bool and context[field] != (verb in verbs):
                raise ProtocolError(f"Selection control {field} disagrees with engine candidates")
    ...
    if len(set(bank_refs)) != len(bank_refs) or not set(slots) <= set(bank_refs):
        raise ProtocolError("Decoder bank does not uniquely cover every legal candidate")
    by_slot = {c["decoder_slot_ref"]: c for c in bank}
    for candidate in candidates:
        if action_semantics(candidate) != action_semantics(by_slot[candidate["decoder_slot_ref"]]):
...
def execution_command(frame: dict, candidate_ref: str) -> dict:
    ...
    command = {"cmd": "execute_candidate", "candidate_ref": candidate_ref,
               "decision_id": routing["decision_id"], "state_version": routing["state_version"]}
...
def clean_frame(frame):
    ...
    result["legal"] = {"candidates": [clean_public({"decoder_bank": [c]})["decoder_bank"][0]
