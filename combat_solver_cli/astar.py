...
def state_key(frame):
    ...
    return fingerprint({'public': frame['public'], 'candidates': [action_semantics(c) for c in frame.get('legal', {}).get('candidates', [])]})


def resolve(frame, semantics):
    matches = [c for c in frame['legal']['candidates'] if action_semantics(c) == semantics]
    if len(matches) != 1: raise ReplayMismatch('Recorded action has no unique native candidate')
    return matches[0]
...
class Prefix:
    ...
    def records(self):
        ...
        while node is not None:
            records.append(dict(before_hash=node.before_hash, action=node.action, actor=node.actor,
                                phase=node.phase, act=node.act, floor=node.floor))
            node = node.parent
        return records[::-1]
...
def from_records(records):
    ...
    for record in records:
        node = Prefix(node, record['before_hash'], deepcopy(record['action']), record['actor'],
                      1 if node is None else node.length + 1, record.get('phase', ''),
                      record.get('act', 0), record.get('floor', 0))
    return node


def append(prefix, frame, candidate, actor):
    p = player(frame)
    return Prefix(prefix, state_key(frame), deepcopy(action_semantics(candidate)), actor,
                  1 if prefix is None else prefix.length + 1, frame['public']['phase'],
                  p.get('act', 0), p.get('floor', 0))
...
def card_value(card, deck):
    ...
    if kind in ('curse', 'status'): return -8.
    if 'STRIKE_' in ident: return -1.4 if card.get('upgraded') else -2.
    if 'DEFEND_' in ident: return -.7 if card.get('upgraded') else -1.3
    stats = card.get('stats') or {}
    def number(key, default=0):
        value = stats.get(key, default)
        return float(value) if isinstance(value, (int, float)) else float(default)
    cost = max(.5, float(card.get('cost', 1) or 0))
    value = 1. + {'rare': 1., 'uncommon': .4}.get(card.get('rarity', '').lower(), 0)
    value += .5*(kind == 'power') + .4*bool(card.get('upgraded'))
    value += min(4., max(0., number('StrengthPower'))*.9 + max(0., number('DexterityPower')))
    hits = max(1., number('Repeat', 1))
    damage = number('Damage', number('CalculationBase'))
    strength = sum(float((c.get('stats') or {}).get('StrengthPower', 0)) for c in deck)
    if damage > 0:
        damage = (damage + .5*strength)*hits
        if card.get('target_type') == 'AllEnemies': damage *= 1.35
        value += max(-.5, min(2.5, damage/cost/5-1))
    if number('Block') > 0: value += max(-.3, min(2., number('Block')/cost/5-1))
    value += min(1.5, .35*(number('WeakPower')+number('VulnerablePower')))
    value += min(1.5, .7*max(0, number('Cards')-cost-1))
    value -= max(0, len(deck)-16)*.4
    value -= sum(c.get('content_id') == ident for c in deck)*.7
    return value


def map_route_cost(frame, start):
    """Minimum public room-risk estimate on the map DAG, including future rests."""
    nodes = {e['ref']: e for e in frame['public']['entities'] if e.get('entity_type') == 'map_node' and e.get('ref')}
    if start not in nodes: return 0.
    children = {ref: [] for ref in nodes}
    for edge in frame['public'].get('relations', []):
        if edge.get('role') == 'map_edge' and edge.get('source') in nodes and edge.get('target') in nodes:
            children[edge['source']].append(edge['target'])
    costs = {'MONSTER': 1.5, 'ELITE': 3., 'RESTSITE': -2., 'REST': -2.,
             'SHOP': .5, 'MERCHANT': .5, 'TREASURE': -.1, 'UNKNOWN': 1., 'BOSS': 0., 'ANCIENT': 0.}
    memo, visiting = {}, set()
    def visit(ref):
        if ref in memo: return memo[ref]
        if ref in visiting: raise ValueError('Public map contains a cycle')
        visiting.add(ref)
        kind = nodes[ref].get('content_id', '').upper()
        tail = min((visit(child) for child in children[ref]), default=0 if kind == 'BOSS' else 20.)
...
def heuristic(frame):
    ...
    remaining = max(0, max((n.get('floor', 0) for n in nodes), default=16) + 1 - floor) + max(0, 3-act)*20
    ...
    boss_rewards = act < 3 and frame['public']['phase'] != 'combat' and any(
        n.get('current') and n.get('content_id', '').lower() == 'boss' for n in nodes)
    if boss_rewards or (act > 1 and floor == 0): hp = max(hp, .75)
    deck = [e for e in entities if e['entity_type'] == 'card' and e.get('zone') == 'deck']
    card_scores = sorted((max(0, card_value(c, [])) for c in deck), reverse=True)
    strength = sum(card_scores[:20])/4 + sum(e['entity_type'] == 'relic' for e in entities)
    deck_bloat = max(0, len(deck)-24)*.75
    phase_cost = {'shop': 4., 'card_select': 3., 'card_reward': 2., 'rewards': 2., 'rest_site': 2.}.get(frame['public']['phase'], 0.)
    return max(0, 3*remaining + 6*(1-hp) + deck_bloat - min(strength, remaining*.25)) + phase_cost
...
def preference(frame, candidate):
    ...
    if verb == 'TAKE_CARD_REWARD': return card_value(source, deck)
    if verb in ('TAKE_REWARD', 'TAKE_RELIC', 'OPEN_CHEST', 'FINISH_SELECTION'): return 3.
    if verb == 'MOVE_TO_NODE':
        local = {'REST': 3 if hp < .65 else 1.5, 'RESTSITE': 3 if hp < .65 else 1.5,
                 'ELITE': -4 if hp < .6 else (1.5 if hp > .8 else 0), 'TREASURE': 3,
                 'SHOP': 1 if p.get('gold', 0)>120 else -2,
                 'MERCHANT': 1 if p.get('gold', 0)>120 else -2, 'UNKNOWN': .7}.get(content, 0.)
        return local - .75*map_route_cost(frame, source.get('ref'))
    if verb == 'CHOOSE_REST_OPTION':
        current = {e.get('ref') for e in entities if e.get('entity_type') == 'map_node' and e.get('current')}
        bosses = {e.get('ref') for e in entities if e.get('entity_type') == 'map_node' and e.get('content_id', '').upper() == 'BOSS'}
        ...
        if 'HEAL' in content or 'REST' in content: return 5 if hp < (.9 if boss_next else .6) else -2
    ...
    if 'BUY' in verb or 'PURCHASE' in verb:
        if content.startswith('CARD.'):
            offered = next((r['target'] for r in frame['public'].get('relations', [])
                            if r.get('source') == source.get('ref') and r.get('role') == 'offers'), None)
            card = next((e for e in entities if e.get('ref') == offered), source)
            return 3 + card_value(card, deck)
        if content.startswith('RELIC.'): return 4.5
        if content.startswith('POTION.'): return 4.2 if hp < .7 else 3.
        if 'REMOVAL' in content: return 4.3
        return 1.5
    if 'REMOVE' in verb: return 2
    if verb in ('CANCEL', 'DISCARD_POTION', 'ABANDON_RUN'): return -12
    if verb == 'LEAVE_ROOM': return 4.
    if verb in ('SKIP', 'SKIP_REWARDS', 'CHOOSE_REWARD_ALTERNATIVE'): return -.5
    return 0.
...
class ReplayWorker:
    ...
    def native_diagnostics(self):
        if self.engine is None or not hasattr(os, 'pread'): return ''
        try:
            fd = self.engine.stderr.fileno()
            size = os.fstat(fd).st_size
            # Read without moving the file offset shared with the child process.
            return os.pread(fd, min(size, 65536), max(0, size-65536)).decode('utf-8', errors='replace')
        except (OSError, ValueError):
            return ''

    def check(self):
        if self.stop_path and self.stop_path.exists(): raise SearchLimit('STOP requested')
        if time.monotonic() >= self.deadline: raise SearchLimit('time budget exhausted')

    def settle(self):
        until = time.monotonic() + 30
        while True:
            self.check()
            if self.prefix and self.prefix.length >= self.max_steps: raise ReplayMismatch('maximum trajectory steps reached')
            validate_frame(self.frame)
            for entity in self.frame.get('public', {}).get('entities', []):
                if entity.get('entity_type') == 'player': self.last_player = entity
            if self.frame['boundary'] != 'waiting': return self.frame
            if time.monotonic() > until: raise ReplayMismatch('native waiting boundary did not settle')
    ...
    def restore(self, prefix):
        ...
        for record in prefix.records() if prefix else []:
            self.settle()
            if state_key(self.frame) != record['before_hash']: raise ReplayMismatch('Native replay public state diverged')
            candidate = resolve(self.frame, record['action'])
            self.frame = self.engine.send(execution_command(self.frame, candidate['candidate_ref']))
            self.replayed += 1
        self.prefix = prefix
        return self.settle()
    ...
    def execute(self, semantics, actor='astar'):
        ...
        frame = self.engine.send(execution_command(self.frame, candidate['candidate_ref']))
    ...
    def combat_and_forced(self):
        ...
        while True:
            self.settle()
            if self.frame['boundary'] == 'terminal': return self.frame
            phase = self.frame['public']['phase']
            if phase == 'combat': in_combat = True
            elif phase not in ('card_select', 'card_reward'): in_combat = False
            elif not in_combat:
            ...
            if in_combat:
                ...
                self.frame = result['frame']; self.solver_steps += 1
...
def save_frontier(path, frontier, character, seed, weight, ascension=10, search_state=None):
    indices, prefixes = {}, []
    def index(node):
        chain = []
        ...
        for node in reversed(chain):
            ...
            prefixes.append(dict(parent=parent, before_hash=node.before_hash, action=node.action, actor=node.actor,
                                 length=node.length, phase=node.phase, act=node.act, floor=node.floor))
            parent = len(prefixes)-1
        return parent
    queue = [[f, tie, g, index(prefix), action] for f, tie, g, prefix, action in frontier]
    write_json(path, dict(schema='astar-frontier-v2', heuristic_version=3, character=character,
                          seed=seed, ascension=ascension, weight=weight, prefixes=prefixes,
                          frontier=queue, search_state=search_state or {}))


def load_frontier(path, character, seed, weight, ascension=10, with_state=False):
    data = json.loads(Path(path).read_text())
    if (data['schema'], data['character'], data['seed'], data.get('ascension', 10), data['weight']) != ('astar-frontier-v2', character, seed, ascension, weight):
        raise ValueError('Checkpoint identity or search weight differs')
    prefixes = []
    for p in data['prefixes']:
        parent = None if p['parent'] == -1 else prefixes[p['parent']]
        prefixes.append(Prefix(parent, p['before_hash'], p['action'], p['actor'], p['length'],
                               p.get('phase', ''), p.get('act', 0), p.get('floor', 0)))
    queue = [(f, tie, g, None if i == -1 else prefixes[i], action) for f, tie, g, i, action in data['frontier']]
    heapq.heapify(queue)
...
def search(config, character, seed, output, *, budget_ms=1000, weight=5., max_expansions=2000,
    ...
    if search_lanes > 1:
        ...
        return search_parallel(config, character, seed, output, budget_ms=budget_ms,
                               weight=weight, max_expansions=max_expansions,
    ...
    if resume:
        ...
        frontier = [(0., 0, 0., None, None)]
    if prefix_path:
        data = json.loads(Path(prefix_path).read_text())
        if (data['character'], data['seed'], data.get('ascension', 10)) != (character, seed, ascension): raise ValueError('Prefix identity differs')
        frontier = [(0., 0, 0., from_records(data['records']), None)]
    ...
    with (output/'search.jsonl').open('w') as journal:
        ...
        try:
            while frontier and expanded < max_expansions:
                ...
                if using_probe:
                    index = next(i for i, node in enumerate(frontier) if node[1] == probe_next)
                ...
                try:
                    ...
                    if pending is not None: worker.execute(pending, 'astar_rollout' if using_probe else 'astar')
                    frame = worker.combat_and_forced()
                    if frame['boundary'] == 'terminal':
                        if frame['public']['outcome']['victory']:
                            winner = worker.prefix; status = 'candidate_victory'; break
                        deaths += 1
                        dead = next((e for e in frame['public'].get('entities', []) if e.get('entity_type') == 'player'), getattr(worker, 'last_player', {}))
                        ...
                        path = output/f"death-act-{dead.get('act')}-floor-{dead.get('floor')}.json"
                        if not path.exists():
                            write_json(path, dict(character=character, seed=seed, ascension=ascension, records=worker.prefix.records() if worker.prefix else [], outcome=frame['public']['outcome']))
                        active = None; continue
                    p = player(frame); progress = (p.get('act', 1), p.get('floor', 0))
                    if progress > best:
                        best = progress
                        write_json(output/'best_prefix.json', dict(character=character, seed=seed, ascension=ascension, records=worker.prefix.records() if worker.prefix else [], progress=best))
                    children = []
                    ...
                    candidates = sorted(frame['legal']['candidates'], key=lambda c: fingerprint(
                        [str(seed), observation_key, action_semantics(c)]))
                    for candidate in candidates:
                        if candidate['verb'] == 'ABANDON_RUN': continue
                        estimate = max(0, heuristic(frame)-.25*preference(frame, candidate))
                        child = (g+.1+weight*estimate, next(serial), g+.1, worker.prefix, action_semantics(candidate))
                        heapq.heappush(frontier, child); children.append(child)
                    if probe_left > 0 and children: probe_next = min(children)[1]
                    event = dict(expanded=expanded, frontier=len(frontier), act=progress[0], floor=progress[1], hp=p['hp'], max_hp=p['max_hp'], deck_size=sum(e.get('entity_type') == 'card' and e.get('zone') == 'deck' for e in frame['public']['entities']), phase=frame['public']['phase'], f=f, prefix_steps=worker.prefix.length if worker.prefix else 0)
                    log(event)
                    if expanded % 10 == 0: print(json.dumps(event), flush=True)
                except (InfrastructureError, SearchLimit): raise
                except (RuntimeError, ValueError, KeyError) as exc:
                    errors += 1; log(dict(expanded=expanded, error=str(exc)))
                    directory = output/'unresolved'; directory.mkdir(exist_ok=True)
                    path = directory/(fingerprint(str(exc))[:16]+'.json')
                    if not path.exists():
                        write_json(path, dict(character=character, seed=seed, ascension=ascension, error=str(exc), records=parent.records() if parent else [], pending_action=pending))
                        diagnostics = getattr(worker, 'native_diagnostics', lambda: '')()
                        if diagnostics: path.with_suffix('.stderr.log').write_text(diagnostics)
                    worker.close()
                active = None
                if expanded % 25 == 0: checkpoint()
            if not frontier and winner is None: status = 'frontier_exhausted_with_unresolved' if errors else 'frontier_exhausted'
            ...
            worker.close()
            if active is not None and winner is None: heapq.heappush(frontier, active)
    ...
    if winner is not None:
        ...
        try:
            ...
            if active is not None: heapq.heappush(frontier, active)
            checkpoint()
