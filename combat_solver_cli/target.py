...
def generate_target(
    ...
    **options,
    ...
    if resume_existing:
        ...
        if (
            manifest.get("mode"),
            manifest.get("target_trajectories"),
            manifest.get("quotas"),
        ...
        (output / "games").mkdir()
        manifest = {
            "mode": "target_verified_trajectories",
            "config": str(config),
    ...
    for trajectory_index in range(len(part_paths), len(schedule)):
        ...
        prior = next(
            (
                Path(entry["output"]) / "frontier.json"
                for entry in reversed(game_record["segments"])
                if (Path(entry["output"]) / "frontier.json").is_file()
            ),
            None,
        )
        if (
    ...
    summary = {
        ...
        "completed": completed,
        "workers": 1,
        "search_lanes": options.get("search_lanes", 2),
        "results": results,
    }
    write_json(output / "summary.json", summary)
    return summary
