...
class RouteDiversity:
    ...
    def _extend(keys, action, act, floor):
        if (
            ...
            or act != 1
            or floor > 5
            or action.get("verb") not in EARLY_VERBS
        ):
            return keys
    ...
    def keys(self, prefix, pending=None):
        ...
        if pending is not None:
            keys = self._extend(
                keys,
                pending,
                prefix.act if prefix else 1,
                prefix.floor if prefix else 0,
            )
        return keys
...
def partition_routes(frontier, lanes):
    ...
    while len(groups) < lanes:
        ...
        if index is None or len(groups[index]) < 2:
            break
