...
class Tree:
    ...
    def add(self, parent, prefix, pending, prior=0.0):
    ...
    def choose(self, children, visits):
        ...
        def score(i):
            n = self.nodes[i]
            if not n.visits:
                return (math.inf, n.prior, self.rng.random())
            uct = n.value_sum / n.visits + self.exploration * math.sqrt(
                math.log(max(2, visits)) / n.visits
            )
            # A diminishing public-action prior; Q remains a backed-up rollout value.
            uct += 0.1 * math.tanh(n.prior / 4) / (n.visits + 1)
    ...
    def schedule_repair(self, path, act, floor):
        ...
        for offset in range(len(path) - 2, -1, -1):
            ...
            if node.decision and any(
                not self.nodes[i].expanded and not self.nodes[i].closed
        ...
        eligible = [
            (i, d)
            for i, d in choices
            if d["act"] < act or (d["act"] == act and d["floor"] <= floor - distance)
        ]
    ...
    def select(self, local_repair=True):
        if local_repair and self.repair_due and self.repair_path:
            ...
            children = [
                i
                for i in node.children
                if not self.nodes[i].expanded and not self.nodes[i].closed
            ]
        ...
        while index is not None:
            path.append(index)
            n = self.nodes[index]
            if not n.expanded or n.terminal is not None:
                return path
    ...
    def rollout_child(self, index, epsilon, frame=None):
        ...
        if frame is not None:
            pool = [i for i in available if self.nodes[i].prior >= best - 0.75]
            p = player(frame)
            ...
            critical = (
                p["hp"] / max(1, p["max_hp"]) < 0.4
                or boss_next
                or "CHOOSE_REST_OPTION" in verbs
                or context in ("remove", "exhaust")
            )
            if critical:
                epsilon *= 0.2
    ...
    def backup(self, path, value):
        for index in reversed(path):
            ...
            if (
                n.expanded
                and n.children
                and all(self.nodes[i].closed for i in n.children)
            ):
                n.closed = True
...
def rollout_value(frame, last_player=None):
    ...
    health = (
        ...
        else p.get("hp", 0) / max(1, p.get("max_hp", 1))
    )
    # Shaped progress, not an estimate of win probability. Victory is strictly higher.
    return min(0.9, max(0.0, 0.8 * progress / 60 + 0.05 * health))
...
def save_tree(path, tree, identity, stats):
    ...
    write_json(
        path,
        {
            ...
            "stats": stats,
            "repair_path": tree.repair_path,
            "repair_due": tree.repair_due,
            "failure_counts": tree.failure_counts,
        },
    )
...
def load_tree(path, identity, exploration):
    ...
    for p in data["prefixes"]:
        prefixes.append(
            Prefix(
                None if p["parent"] == -1 else prefixes[p["parent"]],
                p["before_hash"],
                p["action"],
                p["actor"],
                p["length"],
    ...
    for raw in data["nodes"]:
        n = dict(raw)
        n["prefix"] = None if n["prefix"] == -1 else prefixes[n["prefix"]]
...
def search(
    ...
    *,
    ...
    if not math.isfinite(rollout_epsilon) or not 0 <= rollout_epsilon <= 1:
        raise ValueError("Invalid rollout epsilon")
    if resume and prefix_path:
        raise ValueError("Choose either a tree or a prefix")
    options = {
        "budget_ms": budget_ms,
    ...
    best = (0, 0)
    winner = next((n.prefix for n in tree.nodes if n.terminal == "victory"), None)
    ...
    def checkpoint():
        save_tree(
            ...
            identity,
            {
                "simulations": prior_stats.get("simulations", 0) + simulations,
            ...
            while winner is None and expanded < max_expansions:
                ...
                if not path:
                    status = (
                        ...
                        if any(n.terminal == "unresolved" for n in tree.nodes)
                        else "tree_exhausted"
                    )
                    break
                value = 0.0
                for depth in range(max(1, rollout_decisions)):
                    ...
                    if node.expanded:
                        raise RuntimeError(
                            "MCTS selected an expanded node without a live child"
                        )
                    expanded += 1
                    try:
                        worker.restore(node.prefix)
                        if node.pending is not None:
                            worker.execute(node.pending, "mcts")
                        frame = worker.combat_and_forced()
                        node.prefix = worker.prefix
                        node.pending = None
                        node.expanded = True
                        node.value = value = rollout_value(frame, worker.last_player)
                        if frame["boundary"] == "terminal":
                            if frame["public"]["outcome"]["victory"]:
                                node.terminal = "victory"
                                winner = node.prefix
                            else:
                                node.terminal = "defeat"
                                node.closed = True
                                deaths += 1
                                if local_repair:
                                    tree.schedule_repair(
                                        path,
                                        worker.last_player.get("act", 1),
                                        worker.last_player.get("floor", 0),
                                    )
                            log(
                                {
                                    "expanded": expanded,
                                    ...
                                    "act": worker.last_player.get("act"),
                                    "floor": worker.last_player.get("floor"),
                                }
                            )
                            break
                        p = player(frame)
                        ...
                        candidates = sorted(
                            ...
                            key=lambda c: fingerprint(
                                [seed, observation_key, action_semantics(c)]
                            ),
                        )
                        ...
                        write_json(
                            filename,
                            {
                                ...
                                "seed": seed,
                                "ascension": ascension,
                                "records": node.prefix.records() if node.prefix else [],
                                "pending_action": node.pending,
                                "error": str(exc),
                            },
                        )
                        diagnostics = worker.native_diagnostics()
                        if diagnostics:
                            filename.with_suffix(".stderr.log").write_text(diagnostics)
                        worker.close()
                        break
                    if expanded >= max_expansions or depth + 1 >= max(
                        1, rollout_decisions
                    ):
                        break
                    child = tree.rollout_child(
                        index, rollout_epsilon, frame if risk_aware_rollout else None
                    )
                    if child is None:
                        break
                    path.append(child)
                tree.backup(path, 1.0 if winner is not None else value)
                simulations += 1
