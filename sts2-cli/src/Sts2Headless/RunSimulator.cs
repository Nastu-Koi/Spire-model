...
public partial class RunSimulator
{
    ...
    private volatile TaskCompletionSource<IEnumerable<CardModel>>? _pendingBundleTcs;
    ...
    public Dictionary<string, object?> ExecuteAction(string action, Dictionary<string, object?>? args)
    {
        try
        {
            ...
            if (_pendingOperation.IsActive && HasPendingInteraction()
                && action is not ("select_cards" or "skip_select" or "select_card_reward"
                    or "skip_card_reward" or "select_bundle"))
                return DetectDecisionPoint();
    ...
    private Dictionary<string, object?> DoBuyRelic(Player player, Dictionary<string, object?>? args)
    {
        ...
        try
        {
            ...
            _pendingOperation.Start("shop relic purchase", () => entry.OnTryPurchaseWrapper(inv));
            WaitForPendingOperation();
    ...
    private Dictionary<string, object?> DoRemoveCard(Player player)
    {
        ...
        try
        {
            ...
            _pendingOperation.Start("shop card removal",
                () => removal.OnTryPurchaseWrapper(merchantRoom.GetLocalInventory()));
            WaitForPendingOperation();
            if (HasPendingInteraction())
            {
                WaitForActionExecutor();
    ...
    private Dictionary<string, object?> DoSelectCards(Player player, Dictionary<string, object?>? args)
    {
        ...
        // DetectDecisionPoint resumes the owning operation until completion or
        // another selection. Rest-site transitions happen only after completion.
        return DetectDecisionPoint();
    }
    ...
    private Dictionary<string, object?> DoChooseOption(Player player, Dictionary<string, object?>? args)
    {
        ...
        if (_runState?.CurrentRoom is RestSiteRoom restSiteRoom)
        {
            ...
            try
            {
                ...
                _pendingOperation.Start("rest site option",
                    () => RunManager.Instance.RestSiteSynchronizer.ChooseLocalOption(optionIndex),
                    () =>
                    {
                        WaitForActionExecutor();
                        _syncCtx.Pump();
                        // Preserve the headless transition, but only after all
                        // selections and the option's continuation have completed.
                        if (ReferenceEquals(_runState?.CurrentRoom, restSiteRoom)
            ...
            {
                return ErrorWithTrace("Rest site ChooseLocalOption failed", ex);
            }
            ...
            if (localEvent != null && !localEvent.IsFinished)
            {
                ...
                if (options != null && optionIndex >= 0 && optionIndex < options.Count)
                {
                    try
                    {
                        ...
                        _pendingOperation.Start("event option", () => options[optionIndex].Chosen());
                        WaitForPendingOperation();
                    }
    ...
    internal class HeadlessCardSelector : MegaCrit.Sts2.Core.TestSupport.ICardSelector
    {
            ...
            IReadOnlyList<CardRewardAlternative> alternatives)
        {
            ...
            {
                Interlocked.CompareExchange(ref _pendingReward, null, request);
            }
        }

        public bool HasPendingReward => Volatile.Read(ref _pendingReward) != null;
    ...
    internal static class LocPatches
    {
            ...
            ref Task<IEnumerable<CardModel>> __result)
        {
            ...
            if (sim != null)
            {
                ...
                var request = new TaskCompletionSource<IEnumerable<CardModel>>(TaskCreationOptions.RunContinuationsAsynchronously);
                sim._pendingBundleTcs = request;
                Console.Error.WriteLine($"[SIM] Bundle selection pending: {bundles.Count} packs");
                sim._pendingOperation.NotifyProgress();

                __result = request.Task;
                return false;
            }
