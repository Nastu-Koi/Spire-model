    ...
    for run in iter_runs(path, max_runs):
        ...
        for macro in run["macros"]:
            for step in macro["steps"]:
                ...
                values = {
                    "tokens": len(obs.tokens),
                    "actions": len(obs.slot_refs),
                    "effect_nodes": sum(len(e.nodes) for e in obs.effects),
                    "max_program_length": max(
                        (len(e.nodes) for e in obs.effects), default=0
                    ),
            ...
            group: {
                key: {
                    ...
                    **dict(
                        zip(
                            ("p50", "p90", "p95", "p99"),
                            np.percentile(values, [50, 90, 95, 99]).tolist(),
                        )
                    ),
                }
...
def benchmark(
    ...
    *,
    ...
    runs = (
        list(iter_runs(data))
        if data
        else [demonstration(c, f"benchmark-{c}", 8, 3) for c in CHARACTERS]
    )
    macros = [
        m["steps"]
        for r in runs
        for m in r["macros"]
        if any(not s["forced"] for s in m["steps"])
    ]
    if not macros:
        raise ValueError("No non-forced macros to benchmark")
    batch = [macros[i % len(macros)] for i in range(batch_size)]
    pad_to = batch_pad_size(
        [Sample({"steps": sequence}, "", "", 1.0) for sequence in batch], training
    )
    vocabulary = Vocabulary(
        symbols_from_frames(s["frame"] for m in batch for s in m),
    ...
    def iteration():
        ...
        with precision_context(model, training.precision):
            ...
            loss = torch.stack(
                [
                    -lp + 0.5 * value.square() - 0.001 * entropy
                    for lp, value, entropy in outputs
                ]
            ).mean()
        loss.backward()
        if optimizer_steps:
    ...
    return {
        ...
        "optimizer": training.optimizer if optimizer_steps else None,
        "model": asdict(config),
        ...
        "attention_backends": sorted(
            {m.actual_backend for m in model.modules() if hasattr(m, "actual_backend")}
        ),
    }
