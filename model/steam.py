...
class SteamBridge:
    def __init__(self, directory, timeout=120):
        ...
        try:
            ...
            self.lock.close()
            raise ProtocolError("Another model controller is using this bridge") from None
    ...
    def request(self, command, **payload):
        ...
        temporary.write_text(json.dumps(dict(id=request_id, command=command, **payload)))
        ...
        raise TimeoutError(f"Steam bridge did not respond within {self.timeout}s: {self.directory}")
...
def live_frame(message, selected=()):
    ...
    bank = [snapshot.action(action, index, scope) for index, action in enumerate(raw_bank)]
    public = dict(phase=PHASES[scope], entities=snapshot.entities, relations=snapshot.relations,
                  memory=[], decoder_bank=bank)
    routing = dict(episode_id=message["episode"], decision_id=token, state_version=0)
    candidates = bank
    if selection:
        minimum, maximum = selection["min"], selection["max"]
        require(0 <= minimum <= maximum, "Invalid Steam selection bounds")
        candidates = [bank[i] for i, action in enumerate(raw_bank)
                      if (action["command"] == "select_card" and len(selected) < maximum
        ...
        public["selection_context"] = dict(mode="buffered", min_total=minimum, max_total=maximum,
            selected_refs=[snapshot.refs[x] for x in selected], selected_count=len(selected),
            remaining_required=max(0, minimum-len(selected)), can_finish=len(selected) >= minimum,
            can_cancel=selection["cancelable"], can_skip=False, repetition_allowed=False,
            order_matters=selection.get("ordered", True))
        routing.update(selection_id=token, selection_revision=len(selected), state_version=len(selected),
                       base_public_version=0, action_bank_version=0)
    frame = clean_frame(dict(type="decision_frame", boundary="decision", routing=routing,
                            contract=dict(fixed_ascension=10, training_ready=True,
                                          adapter_version="steam-live-v1", observation_schema="public-state-v1",
                                          action_schema="candidate-v0"), public=public,
                            legal=dict(candidates=candidates), events=[]))
    validate_frame(frame)
    return frame, {bank[i]["candidate_ref"]: action for i, action in enumerate(raw_bank)}
...
def choose_action(policy, message):
    ...
    while True:
        ...
        if action["command"] == "select_card" and message["state"]["legal"].get("selection"):
            selected.append(action["args"]["card_instance_id"])
        elif action["command"] in {"finish_selection", "cancel"}:
            return dict(command="select_cards", args=dict(card_instance_ids=[] if action["command"] == "cancel" else selected))
        else:
            return action
...
def play(checkpoint, directory, device="cpu", timeout=120, max_decisions=10000):
    ...
    try:
        with torch.inference_mode(), precision_context(model, manifest["training"]["precision"]):
            while count < max_decisions:
                message = bridge.request("observe")
                status = message["status"]
                if status == "terminal":
                    return dict(decisions=count, status=status, victory=message.get("victory"))
                if status == "decision":
                    action = choose_action(policy, message)
                    result = bridge.request("execute", token=message["token"], action=action)
                    if result["status"] == "stale":
                        policy.reset()
                        continue
                    require(result["status"] == "executed", "Steam did not execute the action")
                    count += 1
                    last_progress = time.monotonic()
                    print(json.dumps(dict(decision=count, action=action), ensure_ascii=False), flush=True)
                elif status == "waiting":
                    if time.monotonic() - last_progress > timeout:
                        raise TimeoutError("No playable Steam decision: " + message.get("reason", "waiting"))
                    bridge.request("advance")
                    time.sleep(.2)
                else:
                    raise ProtocolError("Unexpected Steam bridge status: " + status)
        return dict(decisions=count, status="decision_limit")
    finally:
        bridge.close()
