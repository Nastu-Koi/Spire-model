...
class RolloutRunner:
    ...
    def run(self, engine, character, seed, *, sample=True, journal=None, _policy=None):
        ...
        trace = {
            ...
            "source": "on_policy" if sample else "evaluation",
            "sampling": "full_distribution" if sample else "argmax",
            "status": "unresolved",
            "macros": [],
            "automatic_steps": 0,
            "initial_reward": 0.0,
            ...
            with (
                ...
                while True:
                    ...
                    if stream:
                        stream.write(
                            json.dumps(
                                {"frame": frame, "reward": reward}, allow_nan=False
                            )
                            + "\n"
                        )
                        stream.flush()
                    ...
                    if boundary == "terminal":
                        victory = bool(
                            frame.get("public", {}).get("outcome", {}).get("victory")
                        )
...
class _InferenceQueue:
    ...
    def policy(self):
        session = SessionPolicy(
            self.runner.model, self.runner.vocabulary, version=self.runner.version
        )
        owner = self
    ...
    def process(self, workers):
        try:
            first = self.requests.get(timeout=0.01)
        except queue.Empty:
            return
        requests = [first]
        ...
        try:
            ...
            with (
                torch.no_grad(),
                precision_context(self.runner.model, self.runner.precision),
            ):
                choices = choose_batch(
                    [r[0] for r in requests], [r[1] for r in requests], sample=first[2]
                )
...
def collect_round(
    engine_factory, runner, seeds, directory, on_run=None, *, workers=1, sample=True
):
    ...
    if any(directory.glob("*.json")):
        raise FileExistsError(
            "Rollout directory already contains runs; choose a new round directory"
        )
    ...
    if assignment.exists():
        raise FileExistsError(
            "Rollout seed assignment already exists; choose a new round directory"
        )
    write_run(assignment, seeds)
    jobs = [
        (character, seed, directory / f"{character}-{i}.json")
        ...
        with ThreadPoolExecutor(
            ...
            try:
                ...
                for future in pending:
                    future.cancel()
    failures = [
        str(p) for p in paths if json.loads(p.read_text())["status"] != "complete"
    ]
    if failures and sample:
        raise ProtocolError(
            f"Round incomplete; no PPO update is permitted. Inspect: {failures}"
        )
    return paths
