...
设计日期：2026-09-19。状态：待实现的架构提案，不是已验证的训练结果。

本方案从任务和数据重新设计；旧模型仅用于核对输入字段覆盖，不沿用其网络、训练流程或权重。依据为本次讨论、引用的「架构设计建议」及 Steam 当前 RunRecorder 记录。最新决定优先：**固定 A10 难度、五角色共享约 0.5B 模型、严格玩家视角、采用局内里程碑奖励，最终验收目标是击败第三幕 Boss。** 难度只在数据契约中校验，不输入模型。
...
第一版采用一个策略、一个整局累计奖励 V。分阶段指奖励的结算时机，不表示切断战斗与战略的回报。模型输入决策阶段，用于理解相同选择在不同场景的含义。字段级设计见 [INPUT_SCHEMA.md](/home/nastukoi/main/sts2-next-design/INPUT_SCHEMA.md)。

## 2. 当前数据的实际边界

只读检查目录：`/mnt/c/Users/Nastu/AppData/Roaming/SlayTheSpire2/run_recorder`。统计的是本次读取时的文件；以后新增记录应重新审计。逐文件结果见 [recorder-audit.json](./recorder-audit.json)。

| 角色 | schema | 文件／对局情况 | 记录结局 | 初步用途 |
| --- | --- | --- | --- | --- |
| 故障机器人 | v1 | 1 文件、1 对局 | 胜利 | 候选示范；需要归并旧式动作生命周期，开局快照有错误 |
| 铁甲战士 | v1 | 1 文件、1 对局 | 失败 | 可审计局部行为；不能把全局失败等同于每步动作错误 |
| 储君 | v2 | 1 文件、1 对局 | 胜利 | 完整示范候选 |
| 亡灵契约师 | v2 | 同一对局的 2 个片段 | 后片段记录胜利 | 先用于可验证的局部行为，不能直接拼成完整轨迹 |
| 静默猎手 | v2 | 1 文件、1 对局 | 胜利 | 完整示范候选 |
| 铁甲战士 | v2 | 1 文件、1 对局 | 胜利 | 完整示范候选 |

全部记录为游戏 `v0.111.0`、A10。当前共 **7 个 JSONL 文件、6 个不同 run_id**，不是 7 条独立完整示范。v2 文件合计 2,792 条根 `decision_committed`。

储君、静默、铁甲的 3 条 v2 胜利记录从开局开始，结尾 `uncommitted_operations=0`，共 2,070 条根操作和 274 条第一层嵌套选择。根操作来源为 431 条 human、1,639 条 combat_solver；按决策域为 429 条 strategic、1,641 条 combat。以上是原始操作数量，**不是已经通过模型输入与动作对齐校验的训练样本数**。自动推进、重复语义、不能恢复候选的记录都可能需要移除。

亡灵第一片段无终局、结尾有 1 个未完成操作；第二片段从第 45 层开始，结尾有 128 个未完成操作。这个计数是完整性异常标记，不等同于已经证实丢失了 128 个有效决策。即使 run_id 相同，也必须证明边界连续、没有回退或重叠，才可传播完整对局标签。

本次 JSON 扫描未发现这些 v2 文件有序号缺口、重复根操作 ID 或 `capture_error`。这只是结构检查，尚未证明每个快照都是可供模型决策的正确时刻，也未证明全部嵌套选择候选完整。

若最终预算约 10 条，目标应是**每角色约 2 条 A10 的完整 v2 轨迹**，优先补足故障、亡灵的完整性，再覆盖不同牌组和路线。胜利轨迹帮助启动，但不应把只有成功结局的数据用于声称价值模型已经校准。
...
\[
e_i = E_{type}+E_{content}+E_{zone}+\mathrm{MLP}_{num}(n_i,k_i)
      +\mathrm{EffectEncoder}(F_i)+\mathrm{ContextEncoder}(c_i)
\]
...
\operatorname{score}_{ij}^{(h)}=\frac{q_i^\top k_j}{\sqrt{d_h}}
 + b^{(h)}_{relation(i,j)} + c^{(h)}_{bucket(floor_j-floor_i)}
\]
...
\[
H=T_\theta([e_{value},E(s_1),\ldots,E(s_N),E(a_1),\ldots,E(a_M)];B).
\]

\[
\ell_a=f_{score}(h_a),\qquad V=g(h_{value}),\qquad
\pi_\theta(a\mid x)=\operatorname{softmax}_{a\in A_{legal}(x)}\ell_a.
\]
...
硬件为用户提供的 RTX PRO 6000、9950X、48GB 内存；实施时实测具体 GPU 显存和软件环境。按 0.5B 估算，BF16 权重约 1GB，常见含 FP32 优化器状态的训练基础存储约 8GB，均为十进制粗算且不含激活、缓存、临时张量或额外策略副本。实际峰值取决于实体数、候选数、attention 实现和微批次。
...
CombatSolver 来源单独保留以供审计。本次只确认记录中的 actor 标签，没有审计求解器的内部信息访问。自动扩增教师数据前，需验证它只依赖公开观测／公开规则；不能假定移除学生输入里的隐藏字段，就证明教师决策也符合严格玩家视角。未核实的旧示范应标记 teacher_visibility=unverified，不作为信息边界合规的证明。
...
\[
J(\theta)=\frac15\sum_{c=1}^{5}P_{\pi_\theta}(\text{第三幕 Boss 胜利}\mid c,A10)
\]

训练采用用户确认的局内里程碑奖励。下表数值是首轮待验证的工程起点，可根据开发评测调整，但一次采样和更新期间必须固定：
...
\[
\gamma=1,\qquad G_t=\sum_{k=t}^{T-1}r_k,\qquad \hat A_t=G_t-V_{old}(x_t).
\]
...
PPO 使用概率比 \(\rho_t=\exp(\log\pi_\theta-\log\pi_{old})\) 与 clipped surrogate；算法依据见 [PPO 论文](https://arxiv.org/abs/1707.06347)。主价值头使用线性输出，以 MSE 拟合未来累计奖励；不使用二元 BCE 或 sigmoid 概率约束。诊断包括 RMSE、explained variance、相对常数基线误差及预测／目标标准差。真实通关率由对局结果统计；若以后增加单独的通关概率辅助头，必须与 PPO 的 V_return 分开命名和监督。

Actor 聚合以“等权角色、每局内部累加决策贡献”为原则，例如：

\[
L_{actor}= -\frac{1}{5H_{ref}}\sum_c\frac1{N_c}\sum_{i=1}^{N_c}
\sum_{t\in\tau_{c,i}}\min(\rho_t\hat A_t,\operatorname{clip}(\rho_t,1-\epsilon,1+\epsilon)\hat A_t)
\]

\(H_{ref}\) 是固定的梯度尺度常量，与单局长度和胜负无关；不能逐局除以自己的动作数而无意改变目标。分桶和微批次只改变计算顺序，保留上述权重。成功重采样、分阶段配额若用于 RL，需要显式分布校正，不能照搬 BC 的平衡采样。
...
从 8 个环境进程开始，实测比较 16、24；集中 GPU 推理，按实体数和候选数分桶。每个环境只加载引擎，不复制 GPU 模型。48GB 系统内存需要限制进程、轨迹队列与未压缩快照，不能只按 CPU 线程数开满。
