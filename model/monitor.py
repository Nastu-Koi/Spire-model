        ...
        try:
            ...
            if status["state"] in {"starting", "running", "collecting", "updating"}:
                try:
                    ...
                    pass
            result.append(dict(name=str(path.parent.relative_to(root)), status=status, history=rows))
...
def serve(root, host="127.0.0.1", port=8765):
    ...
    class Handler(BaseHTTPRequestHandler):
        def do_GET(self):
            ...
            if route == "/api/runs":
                body = json.dumps(snapshot(root), ensure_ascii=False, allow_nan=False).encode()
    ...
    with ThreadingHTTPServer((host, port), Handler) as server:
        print(f"Training monitor: http://{host}:{server.server_port}  root={root}", flush=True)
        try:
