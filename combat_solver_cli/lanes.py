...
def _merge_checkpoints(
    outputs, inputs, results, character, seed, weight, ascension, output, base_state
):
    frontier, states, recovered = [], [], []
    for index, (directory, input_path, result) in enumerate(
        zip(outputs, inputs, results)
    ):
        ...
        path = (
            input_path
            if result["status"] == "lane_error"
            else directory / "frontier.json"
        )
        try:
            queue, state = load_frontier(
                path, character, seed, weight, ascension, with_state=True
            )
        except (OSError, ValueError, KeyError, TypeError, IndexError):
            queue, state = load_frontier(
                input_path, character, seed, weight, ascension, with_state=True
            )
    ...
    frontier = [
        (f, tie, g, prefix, action)
        for tie, (f, _, g, prefix, action) in enumerate(frontier)
    ]
    heapq.heapify(frontier)
    base_failures = int(base_state.get("boss_failures", 0))
    failures = base_failures + sum(
        max(0, int(s.get("boss_failures", 0)) - base_failures) for s in states
    )
    thresholds = [
        s.get("backtrack_before")
        for s in states
    ...
    for state in states:
        for key, count in state.get("route_visits", {}).items():
            route_visits[key] = route_visits.get(key, 0) + max(
                0, count - base_state.get("route_visits", {}).get(key, 0)
            )
    counts = dict(base_state.get("failure_counts", {}))
    for state in states:
        for key, count in state.get("failure_counts", {}).items():
            counts[key] = counts.get(key, 0) + max(
                0, count - base_state.get("failure_counts", {}).get(key, 0)
            )
    state = {
        "boss_failures": failures,
...
def search_parallel(
    ...
    *,
    ...
    warm_expanded = 0
    if resume:
        frontier, base_state = load_frontier(
        ...
        warm = serial_search(
            ...
            budget_ms=budget_ms,
            weight=weight,
            max_expansions=min(max_expansions, search_lanes - 1),
    ...
    shards = (
        partition_routes(frontier, search_lanes)
        if early_route_diversity
        else [ordered[index::search_lanes] for index in range(search_lanes)]
    )
    ...
    for index, shard in enumerate(shards):
        ...
        save_frontier(
            path, shard, character, seed, weight, ascension, search_state=base_state
        )
    ...
    options = {
        ...
        "weight": weight,
        "max_expansions": max(
            1, math.ceil((max_expansions - warm_expanded) / search_lanes)
        ),
    ...
    try:
        with ThreadPoolExecutor(
            max_workers=search_lanes, thread_name_prefix="same-seed-lane"
        ) as pool:
            futures = {
                pool.submit(
                    serial_search,
            ...
            while pending:
                ...
                for future in done:
                    ...
                    if (
                        results[index].get("status") == "verified_victory"
                        and winner is None
                    ):
                        winner = index
    ...
    winner = next(
        (i for i, r in enumerate(results) if r.get("status") == "verified_victory"),
        None,
    )
    recovered = []
    if winner is not None:
        ...
        if "stopped" in statuses or (output / "STOP").exists():
            status = "stopped"
    ...
    summary = {
        ...
        "algorithm": f"{search_lanes}_lane_early_routes_v2"
        if early_route_diversity
        else f"{search_lanes}_lane_sharded_weighted_astar_v1",
        ...
        "reuse_turn_plan": reuse_turn_plan,
        "rollout_decisions": rollout_decisions,
        "boss_failures": state.get(
            "boss_failures",
            max((int(r.get("boss_failures", 0)) for r in results), default=0),
        ),
    ...
    write_json(
        output / "lanes.json",
        {
            "input_sizes": list(map(len, shards)),
            "partition_strategy": "early_routes"
            if early_route_diversity
            else "score_stripes",
            "lanes": results,
        },
