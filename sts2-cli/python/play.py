...
def ensure_setup():
    ...
    if not os.path.isfile(sts2_dll) or not os.path.isfile(
        os.path.join(LIB_DIR, "Sentry.Godot.dll")
    ):
        print(
            t(
                "📦 Game DLLs missing. Running setup...",
                "📦 缺少游戏 DLL，正在运行安装程序……",
            )
        )
        ...
        subprocess.run(
            ["bash", os.path.join(ROOT, "setup.sh"), game_dir], cwd=ROOT, check=True
        )
...
def _find_native_save_dir():
    ...
    if system == "Darwin":
        ...
        patterns = [
            ...
            os.path.join(
                localappdata, "SlayTheSpire2", "steam", "*", "profile*", "saves"
            ),
        ]
    for pat in patterns:
...
def show_native_save(save_path):
    ...
    if room:
        ...
        print(
            f"  {t('Room', '当前房间')}: {room_type_display}"
            + (f" ({_id_to_name(enc)})" if enc else "")
        )

    visited = data.get("visited_map_coords", [])
    if visited:
        print(
            f"  {t('Map', '地图')}: {t('Floor', '层')} {len(visited)} ({len(visited)} {t('nodes visited', '个节点已访问')})"
        )
    ...
    for player in data.get("players", []):
        ...
        print(f"\n  {'─' * 50}")
        print(
            f"  {char_name}  HP: {hp}/{max_hp}  {t('Gold', '金币')}: {gold}  {t('Energy', '能量')}: {energy}"
        )
        ...
        if potions:
            ...
            for p in potions:
                print(
                    f"    🧪 [{p.get('slot_index', '?')}] {_id_to_name(p.get('id', '?'))}"
                )
    ...
    if acts:
        ...
        for i, act in enumerate(acts):
            ...
            print(
                f"    {t('Act', '幕')} {i + 1}: {act_id}  Boss: {boss}  "
...
def resolve_template(text, vars_dict):
    ...
    def replacer(m):
        ...
        if ":" in key and "|" in key:
            ...
            if val is not None:
                ...
                return (
                    forms[0]
                    if int(val) == 1
                    else (forms[1] if len(forms) > 1 else forms[0])
                )
            return f"[{key}]"
        kl = key.lower()
        ...
        if kl == "energyprefix":
            return (
                ""  # prefix only, unit already added by energyIcons handler in desc()
            )
        return f"[{key}]"
    ...
    if show_deck:
        ...
        if cards:
            ...
            for cd in cards:
                ...
                rare = cd.get("rarity")
                rare_part = (
                    f" {c(t(rare, RARITY_ZH.get(rare, rare)), 'dim')}" if rare else ""
                )
                print(
                    f"    {n(cd['name'])}{up} ({cd.get('cost', '?')}) {c(t(cd.get('type', ''), ctype_zh), 'dim')}{rare_part}{suf_part}"
                )
...
def show_combat(state):
    ...
    print(f"\n{'─' * 60}")
    print(
        f"  {c(t(f'Round {rnd}', f'回合 {rnd}'), 'bold')}  {t('Energy', '能量')} {c(f'{energy}/{max_energy}', 'cyan')}  {t('Draw', '抽牌')} {draw}  {t('Discard', '弃牌')} {discard}"
    )
        ...
        for pw in ppowers:
            ...
            if pw_desc and amt:
                pw_desc = resolve_template(
                    pw_desc,
                    {"Amount": abs(amt) if isinstance(amt, (int, float)) else amt},
                )
    ...
    for e in state.get("enemies", []):
        ...
        for it in intents:
            ...
            if itype == "Attack":
                ...
                intent_parts.append(
                    c(
                        t(f"⬆{itype}", f"⬆{'增益' if itype == 'Buff' else '回复'}"),
                        "magenta",
                    )
                )
            elif itype == "Debuff":
    ...
    for card in hand:
        ...
        print(
            f"  {mark} [{card['index']}] {c(n(card['name']), type_color)}{ench_str} ({cost_str}) {stat_str}{suf_part}"
...
def print_card_detail_extension(card, indent="      "):
    ...
    stats = card.get("stats") or {}
    aug_parts = _format_upgrade_preview(
        stats, card.get("after_upgrade"), card.get("cost")
    )
    if aug_parts:
...
def show_card_reward(state):
    ...
    if gold_earned > 0:
        print(
            f"  {c(t('Combat won!', '战斗胜利!'), 'green')} +{c(str(gold_earned), 'yellow')}{t('g', '金')}"
        )
    print(
        f"  {c(t('Card Reward', '卡牌奖励'), 'bold')} — {t('choose one (or skip)', '选一张（或跳过）')}"
    )
    show_player(state.get("player", {}))
    print()
    ...
    for card in cards:
        ...
        cost = card.get("cost", "?")
        type_color = {"Attack": "red", "Skill": "blue", "Power": "magenta"}.get(
            ctype, "reset"
        )
        ...
        suf_part = format_card_suffix_keywords(suf)
        print(
            f"  [{card['index']}] {c(n(card['name']), type_color)} ({cost}) {c(rarity_label, rarity_color)}{suf_part}"
        )
        print_card_detail_extension(card, indent="      ")
    ...
    if cards:
        hi = len(cards) - 1
        print(
            f"  {c(t(f'Pick one card: type index 0–{hi}, or s to skip.', f'请选择一张：输入编号 0–{hi}，或 s 跳过。'), 'yellow')}"
        )
    else:
...
def show_shop(state):
    ...
    for card in state.get("cards", []):
        ...
        suf_part = format_card_suffix_keywords(suf)
        print(
            f"  [{card['index']}] {n(card['name'])} ({cc}) {c(t(card.get('type', '?'), ctype_zh), 'dim')}{suf_part} — {affordable}{t('g', '金')}{sale}"
        )
        print_card_detail_extension(card, indent="      ")
    ...
    if removal_cost:
        affordable = (
            c(str(removal_cost), "green")
            if removal_cost <= gold
            else c(str(removal_cost), "red")
        )
        print(
...
def show_rest_site(state):
    ...
    if ctx:
        print(
            f"  {c(n(ctx.get('act_name', '?')), 'dim')} {t('Floor', '层')} {ctx.get('floor', '?')}"
        )
...
def _render_map(map_data, choice_set=None, choice_indices=None):
    ...
    print(f"  {'─' * width}")
    legend = (
        f"  M={t('Monster', '怪物')} E={t('Elite', '精英')} R={t('Rest', '休息')} "
    ...
    if choice_indices:
        ...
        for i in sorted(inv.keys()):
            ...
            if nd:
                ntype = t(
                    nd.get("type", "?"),
                    NODE_TYPE_ZH.get(nd.get("type", ""), nd.get("type", "?")),
                )
...
def get_input(
    prompt, valid_options=None, state=None, multi_select=False, multi_min=1, multi_max=1
):
...
""")
        ...
        if raw == "map":
            ...
            if hasattr(get_input, "_send"):
                ...
                ctx = state.get("context", {})
                print(
                    f"  {c(n(ctx.get('act_name', '?')), 'bold')} {t('Floor', '层')} {ctx.get('floor', '?')}"
                )
            continue
        if raw == "save":
        ...
        if raw == "saves":
            ...
            if saves:
                ...
                for s in saves:
                    print(
                        f"    {c(s['file'], 'cyan')}  {s['character']}  {t('Seed', '种子')}:{s['seed']}  {t('Actions', '操作数')}:{s['actions']}"
                    )
                print(
        ...
        if raw == "abandon":
            confirm = input(
                f"  {t('Abandon this run? (y/n): ', '放弃本次运行？(y/n): ')}"
            )
        ...
        if valid_options:
            if multi_select and multi_max > 1:
                ...
                if not parts:
                    print(
                        f"  {t('Invalid. Options:', '无效。选项:')} {', '.join(sorted(valid_options))}"
                    )
                    continue
                if len(parts) < multi_min or len(parts) > multi_max:
                    q = card_pick_quantity_hint(multi_min, multi_max)
                    print(
                        f"  {q} — {t(f'Use {multi_min}-{multi_max} comma-separated indices (e.g. 0,1).', f'逗号分隔输入 {multi_min}–{multi_max} 个编号（例 0,1）。')}"
                    )
                    continue
...
def _load_game(save_path):
    ...
    if "actions" not in data:
        print(
            f"{t('Error:', '错误:')} Not a replay save file (missing 'actions' key): {save_path}"
        )
        sys.exit(1)
...
def _quit_with_save(native_save_path, character, seed):
    ...
    try:
        ans = (
            input(f"  {t('Save before quitting? (y/n): ', '退出前是否存档？(y/n): ')}")
            .strip()
            .lower()
        )
    except (EOFError, KeyboardInterrupt):
...
def _show_quit_save_result(result):
    ...
    if save_result and save_result.get("success"):
        ...
        if save_path:
            ...
            print(
                f"  {t('Continue later:', '下次继续:')} python3 play.py --continue {save_path}"
            )
    elif save_result:
        print(
            f"  {c(t('Save failed:', '存档失败:'), 'red')} {save_result.get('message', '?')}"
        )
...
def _writeback_continue_save(send_fn, native_save_path):
    ...
    if result and result.get("success"):
        ...
        print(
            f"  {c(t(f'Save written ({sz // 1024}KB)', f'存档已写入 ({sz // 1024}KB)'), 'dim')}"
        )
    elif result:
        print(
            f"  {c(t('Save failed:', '存档写入失败:'), 'red')} {result.get('message', '?')}"
        )
...
def play(
    ...
    if load_path:
        ...
        print(
            f"  {t('Character', '角色')}: {character}  {t('Seed', '种子')}: {actual_seed}  {t('Actions', '操作数')}: {len(replay_actions)}"
        )
    ...
    def do_save():
        ...
        print(
            f"  {c(t('Saved!', '已存档!'), 'green')} {fname} ({len(action_log)} {t('actions', '步操作')})"
        )
        print(
            f"  {t('Load with:', '读档命令:')} python3 play.py --load {os.path.relpath(save_path, ROOT)}"
        )
    ...
    try:
        ...
        if native_save_path:
            ...
            if replay_actions:
                ...
                for i, cmd in enumerate(replay_actions):
                    ...
                    pct = (i + 1) * 100 // total
                    print(
                        f"\r  {t('Replaying', '回放中')}... {pct}% ({i + 1}/{total})",
                        end="",
                        flush=True,
                    )
                    if not state:
                        ...
                        return
                print(
                    f"\r  {c(t('Replay complete!', '回放完成!'), 'green')}" + " " * 30
                )
                print()
        print(
        ...
        if native_save_path:
            ...
            asc_str = (
                f"  {t('Ascension', '渐进难度')}: {ascension}" if ascension > 0 else ""
            )
            print(
                f"{t('Character', '角色')}: {character}  {t('Seed', '种子')}: {actual_seed}{asc_str}"
            )
        print(
            f"{t('Type', '输入')} {c('help', 'cyan')} {t('for available commands.', '查看可用命令。')}\n"
        )
        ...
        while True:
            ...
            if dec == "game_over":
                ...
                if victory:
                    print(
                        f"  {c('★ ★ ★', 'yellow')}  {c(t('VICTORY!', '胜利!'), 'green')}  {c('★ ★ ★', 'yellow')}"
                    )
                else:
                ...
                print(
                    f"  {t('Act', '幕')}: {state.get('act', '?')} ({act_name})  {t('Floor', '层')}: {floor}"
                )
                print(f"  {t('Character', '角色')}: {n(p.get('name', '?'))}")
                print(
                    f"  HP: {p.get('hp', '?')}/{p.get('max_hp', '?')}  {t('Gold', '金')}: {p.get('gold', '?')}"
                )
                ...
                if relics:
                    ...
                    print(
                        f"  {t('Relics', '遗物')} ({len(relics)}): {', '.join(relic_names)}"
                    )
                ...
                if auto:
                    if len(choices) == 1:
                        ...
                        if hp_ratio < 0.4:
                            pick = next(
                                (ch for ch in choices if ch["type"] == "RestSite"),
                                choices[0],
                            )
                        else:
                            pick = choices[0]
                ...
                if auto:
                    ...
                    if fp == _auto_last_fingerprint:
                        ...
                        if _auto_stuck_count >= 5:
                            print(
                                f"  {c(t('[auto] Stuck state detected, forcing end_turn', '[auto] 检测到卡住状态，强制结束回合'), 'yellow')}"
                            )
                            choice = "e"
                            _auto_stuck_count = 0
                    else:
                    ...
                    choice = get_input(
                        t(
                            "Play card [index], (e)nd turn, (p0) potion",
                            "出牌 [编号], (e)结束回合, (p0)药水",
                        ),
                        set(valid.keys()) | {"help"},
                        state=state,
                    ...
                    if choice == "help":
                        print(
                            f"  {t('Enter card index, e=end turn, p0=use potion 0', '输入卡牌编号，e=结束回合，p0=使用药水0')}"
                        )
                        continue

                if choice == "e":
                    # Track hand before end_turn to detect added status cards
                    [n(cd.get("name", "?")) for cd in hand]
                    state.get("discard_pile_count", 0)
                    ...
                    if state and state.get("decision") == "combat_play":
                        ...
                        if status_cards:
                            ...
                            sc_str = ", ".join(
                                f"{c(name, 'red')}" for name in status_cards
                            )
                            print(
                                f"  ⚠ {t('Status cards in hand:', '手牌中的状态牌:')}: {sc_str}"
                            )
                    ...
                    pot_meta = next(
                        (p for p in pots if p and p.get("index") == pidx), None
                    )
                    if (
                        pot_meta
                        and pot_meta.get("target_type") == "AnyEnemy"
                        and enemies
                    ):
                        tgt = get_input(
                    ...
                    if card.get("target_type") == "AnyEnemy":
                        if len(enemies) == 1:
                            ...
                            args["target_index"] = min(
                                enemies, key=lambda e: e.get("hp", 999)
                            )["index"]
                        else:
                            tgt = get_input(
                                "Target enemy [index]",
                ...
                for b in bundles:
                    ...
                    for cd in b.get("cards", []):
                        ...
                        print(
                            f"    {n(cd['name'])} ({cd.get('cost', '?')}) {c(cd.get('type', ''), 'dim')}{sp}"
                        )
                        print_card_detail_extension(cd, indent="      ")
                ...
                print(
                    f"  {c(t('Choose cards', '选择卡牌'), 'bold')} — {card_pick_quantity_hint(min_sel, max_sel)}"
                )
                show_player(state.get("player", {}))
                print()
                cards = state.get("cards", [])
                for cd in cards:
                    ...
                    rare = cd.get("rarity")
                    rare_part = (
                        f" {c(t(rare, RARITY_ZH.get(rare, rare)), 'dim')}"
                        if rare
                        else ""
                    )
                    _p, sf = split_card_keywords(cd.get("keywords"))
                    sp = format_card_suffix_keywords(sf)
                    print(
                        f"  [{cd['index']}] {n(cd['name'])}{up} ({cd.get('cost', '?')}) {c(ctype_label, 'dim')}{rare_part}{sp}"
                    )
                    print_card_detail_extension(cd, indent="      ")
                ...
                old_deck_cards = [
                    n(cd.get("name", "?"))
                    for cd in state.get("player", {}).get("deck", [])
                ]
                ...
                if state and state.get("player"):
                    ...
                    if added or removed:
                        ...
                        for card_name, cnt in removed.items():
                            parts.append(
                                c(
                                    f"-{card_name}" + (f"x{cnt}" if cnt > 1 else ""),
                                    "red",
                                )
                            )
                ...
                if auto:
                    ...
                    choice = get_input(
                        t(
                            "Buy [index/r0/p0/rm] or (leave)",
                            "购买 [编号/r0/p0/rm] 或 (leave)离开",
                        ),
                        state=state,
                    )
                ...
                if auto:
                    ...
                    heal = next(
                        (o for o in enabled if o.get("option_id") == "HEAL"), None
                    )
                    smith = next(
                        (o for o in enabled if o.get("option_id") == "SMITH"), None
                    )
                ...
                if state and state.get("player"):
                    new_p = state["player"]
                    new_relics = {
                        n(r.get("name", "?")) for r in new_p.get("relics", [])
                    }
                    gained_relics = new_relics - old_relics
                    new_deck_cards = [
                        n(cd.get("name", "?")) for cd in new_p.get("deck", [])
                    ]
                    ...
                    if gained_relics:
                        changes.append(
                            f"{t('Relic', '遗物')}: {', '.join(gained_relics)}"
                        )
                    ...
                    if new_hp != old_hp or new_max_hp != old_max_hp:
                        changes.append(
                            f"{t('HP', '生命')}: {old_hp}/{old_max_hp} → {new_hp}/{new_max_hp}"
                        )
                    if new_gold != old_gold:
                        diff = new_gold - old_gold
                        changes.append(
                            f"{t('Gold', '金')}: {'+' if diff > 0 else ''}{diff}"
                        )
                    if changes:
                        print(
        ...
        while True:
            ...
            if result and result.get("type") == "save_error":
                ...
                try:
                    ans = input(
                        f"  {t('New path (Enter = quit without saving): ', '新路径（回车则不保存退出）: ')}"
                    ).strip()
                except (EOFError, KeyboardInterrupt):
                    ans = ""
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--load", type=str, default=None, help="Load a save file (action replay)"
    )
    parser.add_argument(
        "--saves", action="store_true", help="List available saves and exit"
    )
    parser.add_argument(
        "--save-info",
        type=str,
    ...
    if args.save_info is not None and args.continue_save is not None:
        ...
        if saves:
            ...
            for s in saves:
                ...
                if stype == "native":
                    ...
                    print(
                        f"  {s['file']}  {s['character']}  {t('seed', '种子')}:{s['seed']}  {t('actions', '步操作')}:{s['actions']}"
                    )
            print(f"{'─' * 50}")
            print(
                f"  {t('Replay saves:', '回放存档:')} python3 play.py --load saves/<file>"
            )
            print(
                f"  {t('Native saves:', '原生存档:')} python3 play.py --continue saves/<file>"
            )
        else:
