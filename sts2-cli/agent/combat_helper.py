...
def enemy_threat(en):
    ...
    for it in en.get("intents") or []:
        if it.get("type") in ("Attack", "DeathBlow"):
            hits = it.get("hits", 1)
            atk += it.get("damage", 0) * (max(1, hits))
...
def pick_target(enemies, card_dmg=6):
    ...
    for j, en in enumerate(enemies):
        if en["hp"] <= card_dmg and en.get("intends_attack"):
            if best_oneshot is None or enemy_threat(en) > enemy_threat(
                enemies[best_oneshot]
            ):
                best_oneshot = j
    ...
    for _ in range(200):
        ...
        if dec == "card_select":
            ...
            d = action(
                "select_cards",
                indices=",".join(
                    str(i) for i in range(min(n, len(d.get("cards", []))))
                ),
            )
            continue
        if dec != "combat_play":
...
if __name__ == "__main__":
    ...
    if dec == "card_reward":
        ...
        for c in result.get("cards", []):
            print(
                f"  C{c['index']}: {c['name']['zh']}({c.get('cost', '?')}) {c.get('rarity', '')} {c['type']} {c.get('stats', {})}"
            )
    elif dec == "game_over":
