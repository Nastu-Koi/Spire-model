...
def frame(n=0, boundary="decision", revision=0, **extra):
    return {"type": "decision_frame", "boundary": boundary,
            "contract": {"fixed_ascension": 10, "training_ready": True},
            "routing": {"decision_id": str(revision), "state_version": revision,
...
def test_forced_chain_events_survive_to_terminal():
    engine = Engine([frame(1), frame(1, revision=1, events=[{"reward": 1}]),
...
def test_only_branch_calls_policy_and_sends_current_revision():
    engine = Engine([frame(1), frame(2, revision=1), frame(1, revision=2), frame(boundary="terminal")])
    ...
    assert engine.commands[2] == {"cmd": "execute_candidate", "decision_id": "1", "state_version": 1,
                                  "selection_revision": 1, "candidate_ref": "c1"}
...
@pytest.mark.parametrize("initial,code", [
    ...
    (frame(boundary="error", error={"code": "unsupported_interaction"}), "unsupported_interaction"),
    (frame(1, contract={"fixed_ascension": 10, "training_ready": False}), "prototype_contract_requires_opt_in"),
...
def test_invalid_policy_reference_is_not_sent():
    ...
    with pytest.raises(runner.EnvironmentError, match="policy_selected_unknown_candidate"):
